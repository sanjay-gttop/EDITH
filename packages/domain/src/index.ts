...
export function detectConflict(
  authoritativeResource: Resource,
  incomingEvent: OfflineEvent,
  serverReceivedAt: string = new Date().toISOString(),
  existingConflict?: Conflict,
): { isConflict: boolean; reason?: string; conflict?: Conflict } {
  ...
  const isAlreadyAllocatedToOther =
    authoritativeResource.status !== 'AVAILABLE' &&
  ...
  if (
    ...
    const existingAllocationEvidence: ConflictEvidence = {
...
export function applyConflictResolution(
  ...
  resolvedAt: string = new Date().toISOString(),
): { updatedResource: Resource; updatedConflict: Conflict; auditEvent: DomainEvent } {
  // Strict authorization check
  if (resolverRole !== 'SUPERVISOR' && resolverRole !== 'ADMINISTRATOR') {
    throw new Error(
      `Unauthorized: Role '${resolverRole}' is not permitted to resolve conflicts. Requires SUPERVISOR or ADMINISTRATOR.`,
    );
  }
  ...
  let winnerClaim: ConflictEvidence | undefined;
  let targetState: ResourceStatus = 'CLAIMED';
